...
const CustomExamSession = () => {
  ...
  const slug: string = (location.state as { slug?: string })?.slug ?? '';
  ...
  useEffect(() => {
    if (!session && !isFinishing.current) {
      navigate(slug ? `/exam/custom/${slug}` : '/exam/select', { replace: true });
    }
  }, [session, navigate, slug]);
  ...
  const doSubmit = useCallback(async (isExit = false) => {
    ...
    try {
      ...
      navigate(`/exam/custom/results/${result.id}`);
    } catch {
      isFinishing.current = false;
      setIsSubmitting(false);
    }
  }, [session, isSubmitting, resetSession, deactivateGuard, navigate, setPendingPath]);
  ...
  return (
    <>
      ...
      <Modal isOpen={showExitModal} title={t('exam.exitConfirmTitle')} onClose={handleCancelExit}>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button
            label={isSubmitting ? t('common.saving') : t('exam.exitConfirmButton')}
            variant="danger"
            disabled={isSubmitting}
...
export default CustomExamSession;
